...
<head>
    ...
    <title>Payment Records — JR MARKETING (PVT) LTD</title>
...
<body>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function chequeBadge(status) {
            ...
            return `<span class="payment-method-badge ${cls}">${status === 'Passed' ? '✓ Passed' : '✗ Bounced'}</span>`;
        }
        ...
        function renderTable(records) {
            ...
            tbody.innerHTML = pageRecords.map(rec => {
                const breakdownHtml = rec.payments.map(p => {
                    ...
                    if (p.method === 'Cheque') {
                        ...
                        chequeInfo = parts.length
                            ? `<div class="cheque-detail">${parts.join(' · ')}</div>`
                            : '';
                        chequeInfo += p.chequeStatus ? `<div>${chequeBadge(p.chequeStatus)}</div>` : '';
                    }
                    const note = p.note ? `<div class="cheque-detail" title="${p.note}">📝 ${p.note.substring(0,30)}${p.note.length > 30 ? '…' : ''}</div>` : '';
                ...
                const balanceTxt = balanceAmt <= 0 ? '<span style="color:#16a34a;">✓ Paid</span>' : `<span class="balance-cell-due">${fmtCurrency(balanceAmt)}</span>`;
                ...
                return `<tr>
                    ...
                    <td class="customer-name">${rec.customer || '—'}</td>
            ...
            info.textContent = `Showing ${start + 1}–${Math.min(start + PAGE_SIZE, records.length)} of ${records.length} records`;
