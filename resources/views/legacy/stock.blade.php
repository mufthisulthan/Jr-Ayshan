...
<head>
    ...
    <title>JR MARKETING (PVT) LTD â€” Stock</title>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif;background:#eef1f8;min-height:100vh;display:flex;flex-direction:column}.navbar{background:#0a5c2e;height:52px;display:flex;align-items:center;justify-content:space-between;padding:0 18px 0 0;position:sticky;top:0;z-index:200;box-shadow:0 2px 10px rgba(10,92,46,0.4);flex-shrink:0}.navbar-left{display:flex;align-items:center;height:100%}.nav-sidebar-toggle{width:52px;height:52px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:rgba(255,255,255,0.8);transition:background .2s}.nav-sidebar-toggle:hover{background:rgba(255,255,255,0.1)}.navbar-brand{display:flex;align-items:center;gap:8px;color:#fff;font-weight:800;font-size:15px;letter-spacing:.5px;padding:0 14px 0 8px;white-space:nowrap}.brand-dot{width:9px;height:9px;border-radius:50%;background:#22c55e;display:inline-block}.navbar-right{display:flex;align-items:center;gap:5px}.nav-pos-btn{display:flex;align-items:center;gap:6px;background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.2);border-radius:8px;color:white;padding:0 12px;height:34px;font-size:13px;font-weight:700;cursor:pointer;transition:background .2s}.nav-pos-btn:hover{background:rgba(255,255,255,0.22)}.nav-divider{width:1px;height:22px;background:rgba(255,255,255,0.2);margin:0 4px}.nav-date{color:rgba(255,255,255,0.85);font-size:13px;font-weight:600;padding:0 4px;white-space:nowrap}.nav-icon-btn{width:36px;height:36px;border-radius:8px;background:transparent;border:none;color:rgba(255,255,255,0.75);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .2s}.nav-icon-btn:hover{background:rgba(255,255,255,0.12);color:white}.nav-user{display:flex;align-items:center;gap:7px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.18);border-radius:8px;padding:0 10px;height:34px;color:white;font-size:13px;font-weight:600;cursor:pointer;transition:background .2s}.nav-user:hover{background:rgba(255,255,255,0.2)}.nav-user-avatar{width:24px;height:24px;border-radius:50%;background:#16a34a;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800}
        ...
        .card{background:#fff;border-radius:12px;box-shadow:0 1px 5px rgba(0,0,0,.07);padding:20px;margin-bottom:18px}.card-title{font-size:15px;font-weight:700;color:#111827;margin-bottom:16px}.toolbar{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px}.toolbar input,.toolbar select{height:36px;border:1px solid #d1d5db;border-radius:8px;padding:0 10px;font-size:13px;outline:none}.toolbar input:focus{border-color:#16a34a;box-shadow:0 0 0 3px rgba(22,163,74,.12)}
        ...
        .stats-row{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:18px}.stat-card{background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 5px rgba(0,0,0,.07);text-align:center}.stat-label{font-size:12px;color:#6b7280;font-weight:500}.stat-value{font-size:22px;font-weight:800;color:#111827;margin-top:4px}
        .badge{display:inline-block;padding:2px 10px;border-radius:20px;font-size:11px;font-weight:600}.badge-green{background:#dcfce7;color:#16a34a}.badge-red{background:#fee2e2;color:#dc2626}.badge-yellow{background:#fef9c3;color:#ca8a04}
...
<body>
    ...
    <div class="layout">
        <aside class="sidebar" id="sidebar"><div class="sidebar-inner"><ul class="sidebar-menu">
            ...
            <li class="menu-item open" data-menu="products"><div class="menu-link active" onclick="toggleMenu('products')"><span class="menu-link-left"><span class="menu-icon"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg></span>Products</span><span class="menu-chevron">&#9654;</span></div><ul class="submenu"><li><a class="submenu-link" href="{{ url('/products') }}">&#8227;&nbsp; Products List</a></li><li><a class="submenu-link" href="{{ url('/products') }}#add">&#8227;&nbsp; Add New Product</a></li><li><a class="submenu-link" href="{{ url('/units') }}">&#8227;&nbsp; Units</a></li><li><a class="submenu-link active" href="{{ url('/stock') }}">&#8227;&nbsp; Stock</a></li></ul></li>
        ...
        <main class="main">
            <div class="content-header"><div class="page-title"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg> Stock Management</div></div>
            <div class="content-body">
                <div class="stats-row">
                    <div class="stat-card"><div class="stat-label">Total Products</div><div class="stat-value" id="stat-total">0</div></div>
                    <div class="stat-card"><div class="stat-label">In Stock</div><div class="stat-value" id="stat-in" style="color:#16a34a">0</div></div>
                    <div class="stat-card"><div class="stat-label">Low Stock</div><div class="stat-value" id="stat-low" style="color:#ca8a04">0</div></div>
                    <div class="stat-card"><div class="stat-label">Out of Stock</div><div class="stat-value" id="stat-out" style="color:#dc2626">0</div></div>
                </div>
                <div class="card">
                    <div class="toolbar">
                        <input type="text" id="search-input" placeholder="Search products..." style="flex:1;min-width:200px" oninput="renderList()">
                        <select id="filter-stock" onchange="renderList()"><option value="">All Stock</option><option value="in">In Stock</option><option value="low">Low Stock (â‰¤10)</option><option value="out">Out of Stock</option></select>
                        <button class="btn btn-outline" onclick="exportCSV()">ðŸ“¥ Export</button>
                    </div>
                    <table><thead><tr><th>#</th><th>SKU</th><th>Product</th><th>Category</th><th>Buy Price</th><th>Sell Price</th><th>Qty</th><th>Status</th><th>Stock Value</th></tr></thead><tbody id="stock-tbody"></tbody></table>
                    <div class="empty-state" id="empty-state">No products in stock. Add products first!</div>
                </div>
            </div>
        </main>
    </div>
    ...
    function renderList() {
        ...
        if (sf === 'in') items = items.filter(p => (p.qty||0) > 10);
        if (sf === 'low') items = items.filter(p => (p.qty||0) > 0 && (p.qty||0) <= 10);
        if (sf === 'out') items = items.filter(p => (p.qty||0) <= 0);
        // Stats
        const total = products.length;
        const inStock = products.filter(p => (p.qty||0) > 10).length;
        const low = products.filter(p => (p.qty||0) > 0 && (p.qty||0) <= 10).length;
        ...
        const tbody = document.getElementById('stock-tbody'); const empty = document.getElementById('empty-state');
        if (!items.length) { tbody.innerHTML=''; empty.style.display=''; return; }
        empty.style.display='none';
        tbody.innerHTML = items.map((p,i) => {
            const qty = parseInt(p.qty||0);
            const statusCls = qty <= 0 ? 'badge-red' : qty <= 10 ? 'badge-yellow' : 'badge-green';
            const statusTxt = qty <= 0 ? 'Out of Stock' : qty <= 10 ? 'Low Stock' : 'In Stock';
            const val = qty * parseFloat(p.buyPrice||p.price||0);
            return `<tr><td>${i+1}</td><td style="font-family:monospace;font-size:12px">${p.sku||'â€”'}</td><td><strong>${p.name}</strong></td><td>${p.category||'â€”'}</td><td>${fmtC(p.buyPrice)}</td><td>${fmtC(p.price)}</td><td style="font-weight:700">${qty}</td><td><span class="badge ${statusCls}">${statusTxt}</span></td><td style="font-weight:600">${fmtC(val)}</td></tr>`;
        }).join('');
    }
    function exportCSV() {
        const products = JSON.parse(localStorage.getItem('jr_products') || '[]');
        if (!products.length) { Toast.warning('No data.'); return; }
        const csv = 'SKU,Name,Category,BuyPrice,SellPrice,Qty,StockValue\n'+products.map(p=>`"${p.sku||''}","${p.name}","${p.category||''}",${p.buyPrice||0},${p.price||0},${p.qty||0},${(p.qty||0)*(p.buyPrice||0)}`).join('\n');
        const b=new Blob([csv],{type:'text/csv'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='stock.csv';a.click();
    }
    </script>
    @livewireScripts
</body>
