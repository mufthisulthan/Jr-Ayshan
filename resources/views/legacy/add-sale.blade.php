...
<head>
    ...
    <title>JR MARKETING (PVT) LTD â€” Add Sale</title>
...
<body>
            ...
            <div class="content-body">
                ...
                <div class="card">
                    ...
                    <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:18px">
                        ...
                        <button class="btn btn-primary" style="background:#0369a1" onclick="saveSale('draft')">ðŸ“‹ Save Draft</button>
                        <button class="btn btn-primary" onclick="saveSale('completed')">ðŸ’¾ Complete Sale</button>
                    </div>
                </div>
            </div>
        </main>
    ...
    function loadDropdowns() {
        ...
        products.forEach(p => { const o = document.createElement('option'); o.value = p.id||p.sku; o.textContent = p.name + (p.sku?' ('+p.sku+')':'') + ' â€” '+fmtC(p.price); ps.appendChild(o); });
    }
    function addItem() {
        ...
        saleItems.push({ product: sel.options[sel.selectedIndex].textContent.split(' â€” ')[0], productId: sel.value, qty, price, subtotal: qty*price });
    ...
    function renderItems() {
        ...
        tbody.innerHTML = saleItems.map((it,i)=>`<tr><td>${it.product}</td><td>${it.qty}</td><td>${fmtC(it.price)}</td><td>${fmtC(it.subtotal)}</td><td><button style="background:#fee2e2;color:#dc2626;border:none;border-radius:6px;padding:4px 10px;cursor:pointer;font-weight:600" onclick="removeItem(${i})">âœ•</button></td></tr>`).join('');
        calcTotal();
    }
    function calcTotal() {
