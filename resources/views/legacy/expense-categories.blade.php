...
<head>
    ...
    <title>JR MARKETING (PVT) LTD â€” Expense Categories</title>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif;background:#eef1f8;min-height:100vh;display:flex;flex-direction:column}.navbar{background:#0a5c2e;height:52px;display:flex;align-items:center;justify-content:space-between;padding:0 18px 0 0;position:sticky;top:0;z-index:200;box-shadow:0 2px 10px rgba(10,92,46,0.4);flex-shrink:0}.navbar-left{display:flex;align-items:center;height:100%}.nav-sidebar-toggle{width:52px;height:52px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:rgba(255,255,255,0.8);transition:background .2s}.nav-sidebar-toggle:hover{background:rgba(255,255,255,0.1)}.navbar-brand{display:flex;align-items:center;gap:8px;color:#fff;font-weight:800;font-size:15px;letter-spacing:.5px;padding:0 14px 0 8px;white-space:nowrap}.brand-dot{width:9px;height:9px;border-radius:50%;background:#22c55e;display:inline-block}.navbar-right{display:flex;align-items:center;gap:5px}.nav-pos-btn{display:flex;align-items:center;gap:6px;background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.2);border-radius:8px;color:white;padding:0 12px;height:34px;font-size:13px;font-weight:700;cursor:pointer;transition:background .2s}.nav-pos-btn:hover{background:rgba(255,255,255,0.22)}.nav-divider{width:1px;height:22px;background:rgba(255,255,255,0.2);margin:0 4px}.nav-date{color:rgba(255,255,255,0.85);font-size:13px;font-weight:600;padding:0 4px;white-space:nowrap}.nav-icon-btn{width:36px;height:36px;border-radius:8px;background:transparent;border:none;color:rgba(255,255,255,0.75);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .2s}.nav-icon-btn:hover{background:rgba(255,255,255,0.12);color:white}.nav-user{display:flex;align-items:center;gap:7px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.18);border-radius:8px;padding:0 10px;height:34px;color:white;font-size:13px;font-weight:600;cursor:pointer;transition:background .2s}.nav-user:hover{background:rgba(255,255,255,0.2)}.nav-user-avatar{width:24px;height:24px;border-radius:50%;background:#16a34a;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800}
        ...
        .inline-form{display:flex;gap:10px;align-items:end;flex-wrap:wrap;margin-bottom:18px}.form-group{display:flex;flex-direction:column;gap:4px}.form-group label{font-size:12px;font-weight:600;color:#374151}.form-group input{height:36px;border:1px solid #d1d5db;border-radius:8px;padding:0 10px;font-size:13px;outline:none}.form-group input:focus{border-color:#16a34a;box-shadow:0 0 0 3px rgba(22,163,74,.12)}
        .btn{height:36px;padding:0 16px;border:none;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:6px;transition:background .2s}.btn-primary{background:#16a34a;color:#fff}.btn-primary:hover{background:#15803d}
        ...
        @media(max-width:768px){.sidebar{display:none}.sidebar.mobile-open{display:flex;position:fixed;left:0;top:52px;height:calc(100vh - 52px);z-index:300;width:240px;box-shadow:4px 0 20px rgba(0,0,0,.2)}.nav-date{display:none}}
    </style>
...
<body>
    ...
    <div class="layout">
        ...
        <main class="main">
            <div class="content-header"><div class="page-title">ðŸ·ï¸ Expense Categories</div></div>
            <div class="content-body">
                <div class="card">
                    <div class="inline-form">
                        <div class="form-group" style="flex:1;min-width:200px"><label>Category Name *</label><input type="text" id="cat-name" placeholder="e.g. Rent, Utilities, Salary"></div>
                        <div class="form-group" style="flex:0 0 120px"><label>Code</label><input type="text" id="cat-code" placeholder="EXP-001"></div>
                        <button class="btn btn-primary" onclick="addCategory()">âž• Add Category</button>
                    </div>
                    <table><thead><tr><th>#</th><th>Category</th><th>Code</th><th>Expenses</th><th>Total Spent</th><th>Actions</th></tr></thead><tbody id="cat-tbody"></tbody></table>
                    <div class="empty-state" id="empty-state">No expense categories. Add one above!</div>
                </div>
            </div>
        </main>
    </div>
    ...
    <script>
    const STORAGE_KEY = 'jr_expense_categories';
    const defaults = ['Rent','Utilities','Salary','Transport','Office Supplies','Marketing','Repairs','Insurance','Miscellaneous'];
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (!localStorage.getItem(STORAGE_KEY)) {
            const cats = defaults.map((n,i) => ({ id: 'ECAT-'+(Date.now()+i), name: n, code: 'EXP-'+(i+1).toString().padStart(3,'0') }));
    ...
    function addCategory() {
        ...
        if (all.some(c=>c.name.toLowerCase()===name.toLowerCase())) { Toast.warning('Already exists'); return; }
        all.push({ id:'ECAT-'+Date.now(), name, code: code||('EXP-'+(all.length+1).toString().padStart(3,'0')) });
        localStorage.setItem(STORAGE_KEY, JSON.stringify(all));
        document.getElementById('cat-name').value=''; document.getElementById('cat-code').value='';
        Toast.success('Category added!'); renderList();
    }
    function deleteCategory(idx) {
        ...
        all.splice(idx,1);
        localStorage.setItem(STORAGE_KEY, JSON.stringify(all));
        Toast.success('Deleted'); renderList();
    }
    function renderList() {
        ...
        if (!cats.length) { tbody.innerHTML=''; empty.style.display=''; return; }
        empty.style.display='none';
        tbody.innerHTML = cats.map((c,i) => {
            const catExpenses = expenses.filter(e => (e.category||'').toLowerCase() === c.name.toLowerCase());
            const total = catExpenses.reduce((s,e) => s + parseFloat(e.amount||0), 0);
            return `<tr><td>${i+1}</td><td><strong>${c.name}</strong></td><td style="font-family:monospace;font-size:12px">${c.code||'â€”'}</td><td>${catExpenses.length}</td><td style="font-weight:600">${fmtC(total)}</td><td><button style="background:#fee2e2;color:#dc2626;border:none;border-radius:6px;padding:4px 10px;cursor:pointer;font-size:12px;font-weight:600" onclick="deleteCategory(${i})">ðŸ—‘</button></td></tr>`;
        }).join('');
    }
    </script>
    @livewireScripts
