...
<head>
    ...
    <title>JR MARKETING (PVT) LTD â€” Sale Return</title>
...
<body>
    ...
    <div class="layout">
        ...
        <main class="main">
            <div class="content-header"><div class="page-title">ðŸ”„ Sale Return</div></div>
            <div class="content-body">
                <div class="card">
                    ...
                    <div style="margin-top:16px;display:flex;gap:10px">
                        <button class="btn btn-danger" onclick="processReturn()">ðŸ”„ Process Return</button>
                    </div>
                </div>
                <div class="card">
    ...
    function loadSales() {
        ...
        sales.forEach(s => { const o = document.createElement('option'); o.value = s.id; o.textContent = s.id+' â€” '+(s.customer||'Walk-in')+' â€” '+fmtC(s.totalPayable||s.total||0); sel.appendChild(o); });
    }
    function loadSaleDetails() {
        ...
        det.style.display='';
        document.getElementById('sale-items-tbody').innerHTML = sale.items.map((it,i)=>`<tr><td>${it.name||it.product||'â€”'}</td><td>${it.qty||0}</td><td>${fmtC(it.price||it.unitPrice||0)}</td><td><input type="number" id="retqty-${i}" min="0" max="${it.qty||0}" value="0" style="width:60px;height:30px;border:1px solid #d1d5db;border-radius:6px;text-align:center"></td></tr>`).join('');
    }
