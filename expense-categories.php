...
<head>
    ...
    <title>JR MARKETING (PVT) LTD — Expense Categories</title>
...
<body>
    ...
    <div class="layout">
        ...
        <main class="main">
            <div class="content-header"><div class="page-title">🏷️ Expense Categories</div></div>
    ...
    function renderList() {
        ...
        tbody.innerHTML = cats.map((c,i) => {
            ...
            return `<tr><td>${i+1}</td><td><strong>${c.name}</strong></td><td style="font-family:monospace;font-size:12px">${c.code||'—'}</td><td>${catExpenses.length}</td><td style="font-weight:600">${fmtC(total)}</td><td><button style="background:#fee2e2;color:#dc2626;border:none;border-radius:6px;padding:4px 10px;cursor:pointer;font-size:12px;font-weight:600" onclick="deleteCategory(${i})">🗑</button></td></tr>`;
        }).join('');
    }
    </script>
</body>
