...
<head>
    ...
    <title>JR MARKETING (PVT) LTD — Stock</title>
...
<body>
    ...
    <div class="layout">
        <aside class="sidebar" id="sidebar"><div class="sidebar-inner"><ul class="sidebar-menu">
            ...
            <li class="menu-item open" data-menu="products"><div class="menu-link active" onclick="toggleMenu('products')"><span class="menu-link-left"><span class="menu-icon"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg></span>Products</span><span class="menu-chevron">&#9654;</span></div><ul class="submenu"><li><a class="submenu-link" href="products.php">&#8227;&nbsp; Products List</a></li><li><a class="submenu-link" href="products.php#add">&#8227;&nbsp; Add New Product</a></li><li><a class="submenu-link" href="units.php">&#8227;&nbsp; Units</a></li><li><a class="submenu-link active" href="stock.php">&#8227;&nbsp; Stock</a></li></ul></li>
            ...
            <div class="content-body">
                ...
                <div class="card">
                    <div class="toolbar">
                        ...
                        <select id="filter-stock" onchange="renderList()"><option value="">All Stock</option><option value="in">In Stock</option><option value="low">Low Stock (≤10)</option><option value="out">Out of Stock</option></select>
    ...
    function renderList() {
        ...
        tbody.innerHTML = items.map((p,i) => {
            ...
            return `<tr><td>${i+1}</td><td style="font-family:monospace;font-size:12px">${p.sku||'—'}</td><td><strong>${p.name}</strong></td><td>${p.category||'—'}</td><td>${fmtC(p.buyPrice)}</td><td>${fmtC(p.price)}</td><td style="font-weight:700">${qty}</td><td><span class="badge ${statusCls}">${statusTxt}</span></td><td style="font-weight:600">${fmtC(val)}</td></tr>`;
        }).join('');
    }
    function exportCSV() {
        const products = JSON.parse(localStorage.getItem('jr_products') || '[]');
